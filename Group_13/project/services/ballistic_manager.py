...
class BallisticManager:
    ...
    def calculate_trajectory(self,
        ...
        projectile = Projectile(
            ...
            drag_model=projectile_params.get("drag_model", DragModel.G1)
        )
        ...
        formatted_telemetry = {
            ...
            "latitude": [d["latitude"] for d in raw_telemetry],
            "longitude": [d["longitude"] for d in raw_telemetry],
            "gravity": [d["gravity"] for d in raw_telemetry]
        }
        
        summary = simulation_result["summary"]
        summary["drag_model"] = projectile.drag_model.value
    ...
    def solve_firing_solution(self,
                              ...
                              status_callback=None) -> Tuple[float, float]:
        """Solves for required Velocity and Heading to hit target."""
        # Target Data
        ...
        # Initial Guesses
        gravity = get_gravity_at_location(lat, alt)
        ...
        v_guess = math.sqrt(target_dist_m * gravity / sin_2theta)
        ...
        for h_iter in range(max_heading_iter):
            ...
            res = self.calculate_trajectory(lat, lon, alt, v_solved, heading_guess, climb_angle, projectile_params, progress_callback=progress_callback)
    ...
    def _solve_velocity_for_heading(self, lat, lon, alt, heading, climb, params, target_dist, v_guess, progress_wrapper=None):
        ...
        for i in range(MAX_SOLVER_ITERATIONS_VELOCITY):
            # V0
            res0 = self.calculate_trajectory(lat, lon, alt, v0, heading, climb, params, progress_callback=progress_wrapper)
            ...
            # V1
            res1 = self.calculate_trajectory(lat, lon, alt, v1, heading, climb, params, progress_callback=progress_wrapper)
